...
export default function EmployeesPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // تحميل الفروع من جدول Def_CompanyStructure
      ...
      // تحميل الموظفين من جدول Dealing_Employees
      ...
      console.log("تم تحميل الفروع:", branchesData.length)
      console.log("تم تحميل الموظفين:", employeesData.length)
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `الموظفين_${new Date().toLocaleDateString('ar-EG')}.csv`)
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      {/* شريط العنوان والأزرار - مطابق للصورة */}
      <div className="w-full overflow-x-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3 min-w-max">
        ...
        <div className="bg-gray-100 px-4 py-2 rounded-lg w-full md:w-auto text-center">
          <h1 className="text-xl font-bold text-gray-700">عرض الموظفين</h1>
        </div>
        <Button
          onClick={() => router.push("/admin/dealings/employees/form")}
      ...
      <Card>
        <CardContent className="p-6">
          <div className="w-full overflow-x-auto">
            <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 md:gap-4 min-w-max">
            ...
            {/* فلتر الفرع */}
            <Select value={selectedBranchId === null ? "all" : String(selectedBranchId)} onValueChange={(value) => setSelectedBranchId(value === "all" ? null : Number(value))}>
              ...
              <SelectContent>
                ...
                {branches.map((b) => (
                  <SelectItem key={b.ID} value={String(b.ID)}>{b.Name || `فرع ${b.ID}`}</SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      {/* جدول البيانات */}
      <Card>
        <CardContent className="p-0 overflow-x-auto overflow-y-auto max-h-[65vh]">
          <Table className="min-w-[1000px]">
            <TableHeader className="sticky top-0 bg-white z-10">
              <TableRow>
                {/* عمود الإجراءات في المقدمة (أول عمود من اليمين) */}
              ...
              {loading ? (
                ...
                currentEmployees.map((employee) => (
                  <TableRow
                    ...
                    onDoubleClick={() => router.push(`/admin/dealings/employees/form?id=${employee.ID}`)}
                  >
                    {/* خلية الإجراءات في المقدمة */}
                    <TableCell className="text-right">
                      <div className="flex items-center space-x-2 space-x-reverse">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            router.push(`/admin/dealings/employees/form?id=${employee.ID}`)
                          }}
                          title="تعديل"
                        >
                    ...
                    <TableCell className="text-right max-w-[200px] md:max-w-xs truncate" title={employee.Address || ""}>
                      {employee.Address || "-"}
                    </TableCell>
                    <TableCell className="text-right">
                    ...
                    <TableCell className="text-right">
                      <Badge variant={employee.IsBindShop ? "default" : "secondary"}>
                        {employee.IsBindShop ? "متجر" : "عادي"}
                      </Badge>
                    </TableCell>
                  </TableRow>
                ))
