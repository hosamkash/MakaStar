  ...
  useEffect(() => {
    const initializeForm = async () => {
      if (id) {
        // وضع التعديل
        try {
          const docRef = doc(db, "Def_Sections", id)
          ...
          notify.error("حدث خطأ أثناء جلب بيانات القسم")
        }
      } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const isCodeValid = await validateCodeUniqueness(
        DEFINITION_COLLECTIONS.sections,
        numericId,
        id || undefined
      )
      ...
      const docRef = doc(db, "Def_Sections", formData.id)
      ...
      if (id) {
        ...
        notify.success("تم تحديث القسم بنجاح")
      } else {
        // إضافة جديدة
        await setDoc(docRef, {
          ID: numericId,
        ...
        notify.success("تم إضافة القسم بنجاح")
      }

      router.push("/admin/definitions/sections?refresh=true")
    } catch (error) {
      console.error("Error saving section:", error)
      notify.error("حدث خطأ أثناء حفظ القسم")
    }
  }
