...
export default function SectionsPage() {
  ...
  const { settings, loading: loadingSettings, updateItemsPerPage, updateLastSelectedItem } = useScreenSettings("/admin/definitions/sections")
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setLoading(true)
        const itemsCollection = collection(db, "Def_Sections")
        ...
        notify.error("حدث خطأ أثناء جلب بيانات الأقسام")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "Def_Sections", id))
      setItems((prevItems) => prevItems.filter((item) => item.id !== id))
      setSelectedItem(null)
      notify.success("تم حذف القسم بنجاح")
    } catch (error) {
      console.error("Error deleting section:", error)
      notify.error("حدث خطأ أثناء حذف القسم")
    }
  }
  ...
  const handleEdit = async (id: string) => {
    ...
    router.push(`/admin/definitions/sections/form?id=${id}`)
  }

  const requestSort = (key: keyof Section) => {
  ...
  const SortableHeader = ({ columnKey, children, className }: { columnKey: keyof Section; children: React.ReactNode; className?: string }) => (
  ...
  const actionButtons = [
    { label: "جديد", icon: Plus, href: "/admin/definitions/sections/form" },
    { 
      label: "تعديل", 
      icon: Edit, 
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader title="عرض الأقسام" actionButtons={actionButtons} />
