...
export default function JobsPage() {
  ...
  const { settings, loading: loadingSettings, updateItemsPerPage, updateLastSelectedItem } = useScreenSettings("/admin/definitions/jobs")
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        notify.error("حدث خطأ أثناء جلب بيانات الوظائف")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "Def_Jobs", id))
      setItems((prevItems) => prevItems.filter((item) => item.id !== id))
      setSelectedItem(null)
      notify.success("تم حذف الوظيفة بنجاح")
    } catch (error) {
      console.error("Error deleting job:", error)
      notify.error("حدث خطأ أثناء حذف الوظيفة")
    }
  }
  ...
  const handleEdit = async (id: string) => {
    ...
    router.push(`/admin/definitions/jobs/form?id=${id}`)
  }
  ...
  const SortableHeader = ({ columnKey, children, className }: { columnKey: keyof Job; children: React.ReactNode; className?: string }) => (
  ...
  const actionButtons = [
    { label: "جديد", icon: Plus, href: "/admin/definitions/jobs/form" },
    { 
      label: "تعديل", 
      icon: Edit, 
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader title="عرض الوظائف" actionButtons={actionButtons} />
