...
export default function UnitFormPage() {
    ...
    const fetchMaxValues = async () => {
      if (!id) { // فقط في حالة إضافة جديدة
        try {
          ...
          unitsSnapshot.docs.forEach((doc: QueryDocumentSnapshot<DocumentData>) => {
            const data = doc.data()
  ...
  const handleSave = async () => {
    try {
      ...
      if (!formData.Name.trim()) {
        alert('يجب إدخال اسم الوحدة')
        return
      }

      // التحقق من وجود سجل بنفس ID في قاعدة البيانات
      const docRef = doc(db, 'Def_Units', String(formData.ID))
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <PageHeader title={id ? "تعديل الوحدة" : "إضافة وحدة جديدة"} actionButtons={actionButtons} />
      <Card>
