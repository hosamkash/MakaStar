...
export default function TreasuriesPage() {
  ...
  const { settings, loading: loadingSettings, updateItemsPerPage, updateLastSelectedItem } = useScreenSettings("/admin/definitions/treasuries")
  ...
  const [selectedItem, setSelectedItem] = useState<Treasury | null>(null)
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        notify.error("حدث خطأ أثناء جلب بيانات الخزائن")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "Def_Treasures", id))
      setItems((prevItems) => prevItems.filter((item) => item.id !== id))
      setSelectedItem(null)
      notify.success("تم حذف الخزينة بنجاح")
    } catch (error) {
      console.error("Error deleting treasury:", error)
      notify.error("حدث خطأ أثناء حذف الخزينة")
    }
  }
  ...
  const handleEdit = async (id: string) => {
    ...
    router.push(`/admin/definitions/treasuries/form?id=${id}`)
  }

  const requestSort = (key: keyof Treasury) => {
  ...
  const SortableHeader = ({ columnKey, children }: { columnKey: keyof Treasury; children: React.ReactNode }) => (
  ...
  const actionButtons = [
    { label: "جديد", icon: Plus, href: "/admin/definitions/treasuries/form" },
    { 
      label: "تعديل", 
      icon: Edit, 
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader title="عرض الخزائن" actionButtons={actionButtons} />
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <SortableHeader columnKey="Balance">الرصيد</SortableHeader>
              ...
              <TableBody>
                {paginatedData.map((item) => (
                  ...
                  >
                    ...
                    <TableCell>{item.Balance.toLocaleString('ar-EG')}</TableCell>
                    <TableCell className="text-center">
                      <Checkbox checked={item.IsActive} disabled />
                    </TableCell>
                    <TableCell className="text-center">
                      <Checkbox checked={item.IsBindBranch} disabled />
                    </TableCell>
                    <TableCell className="text-center">
