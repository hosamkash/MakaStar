...
export default function SizeFormPage() {
  ...
  const [formData, setFormData] = useState<SizeItem>({
    id: "",
    ID: "",
    Name: "",
    IsActive: true
  ...
  useEffect(() => {
    const initializeForm = async () => {
      if (id) {
        // وضع التعديل
        try {
          const docRef = doc(db, "Def_Sizes", id)
          ...
          notify.error("حدث خطأ أثناء جلب بيانات المقاس")
        }
      } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const docRef = doc(db, "Def_Sizes", formData.id)
      ...
      if (id) {
        ...
        notify.success("تم تحديث المقاس بنجاح")
      } else {
        // إضافة جديدة
        await setDoc(docRef, {
          ID: numericId,
        ...
        notify.success("تم إضافة المقاس بنجاح")
      }

      router.push("/admin/definitions/sizes?refresh=true")
    } catch (error) {
      console.error("Error saving size:", error)
      notify.error("حدث خطأ أثناء حفظ المقاس")
    }
  }
