...
export default function ColorsPage() {
  ...
  const { settings, loading: loadingSettings, updateItemsPerPage, updateLastSelectedItem } = useScreenSettings("/admin/definitions/colors")
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        notify.error("حدث خطأ أثناء جلب بيانات الألوان")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "Def_Colors", id))
      setItems((prevItems) => prevItems.filter((item) => item.id !== id))
      setSelectedItem(null)
      notify.success("تم حذف اللون بنجاح")
    } catch (error) {
      console.error("Error deleting color:", error)
      notify.error("حدث خطأ أثناء حذف اللون")
    }
  }
  ...
  const handleEdit = async (id: string) => {
    ...
    router.push(`/admin/definitions/colors/form?id=${id}`)
  }

  const requestSort = (key: keyof ColorItem) => {
  ...
  const SortableHeader = ({ columnKey, children, className }: { columnKey: keyof ColorItem; children: React.ReactNode; className?: string }) => (
  ...
  const actionButtons = [
    { label: "جديد", icon: Plus, href: "/admin/definitions/colors/form" },
    { 
      label: "تعديل", 
      icon: Edit, 
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader title="عرض الألوان" actionButtons={actionButtons} />
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <TableHead className="text-center font-bold text-base">اللون</TableHead>
                  ...
                  >
                    ...
                    <TableCell className="text-center">
                      <div className="inline-flex items-center gap-2">
                        <span className="font-mono text-xs text-gray-600">{item.ColorHex}</span>
                        <span className="inline-block w-5 h-5 rounded border" style={{ backgroundColor: item.ColorHex || "#000000" }} />
                      </div>
                    </TableCell>
                    <TableCell className="text-center">
