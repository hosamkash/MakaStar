...
type SortConfig = {
  key: keyof OfferByProducts | null
  direction: "ascending" | "descending"
}

export default function OffersByProductsPage() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const refresh = searchParams.get("refresh") === "true"
  const { settings, loading: loadingSettings, updateItemsPerPage, updateLastSelectedItem } = useScreenSettings("/admin/definitions/OffersByProducts")
  
  const [items, setItems] = useState<OfferByProducts[]>([])
  ...
  const [selectedItem, setSelectedItem] = useState<OfferByProducts | null>(null)
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        console.error("Error fetching offers by products:", error)
        notify.error("حدث خطأ أثناء جلب بيانات عروض المنتجات")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "Def_OffersByProducts", id))
  ...
  const handleEdit = async (id: string) => {
    ...
    router.push(`/admin/definitions/OffersByProducts/form?id=${id}`)
  }

  const requestSort = (key: keyof OfferByProducts) => {
  ...
  const SortableHeader = ({ columnKey, children, className }: { columnKey: keyof OfferByProducts; children: React.ReactNode; className?: string }) => (
  ...
  const actionButtons = [
    { label: "جديد", icon: Plus, href: "/admin/definitions/OffersByProducts/form" },
    { 
      label: "تعديل", 
      icon: Edit, 
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader title="عرض عروض المنتجات" actionButtons={actionButtons} />
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <SortableHeader columnKey="TotalValue" className="text-center">القيمة الإجمالية</SortableHeader>
                  <SortableHeader columnKey="TotalValueAfterOffer" className="text-center">القيمة بعد العرض</SortableHeader>
                  <SortableHeader columnKey="ProductsCount" className="text-center">عدد المنتجات</SortableHeader>
              ...
              <TableBody>
                {paginatedData.map((item) => (
                  ...
                  >
                    ...
                    <TableCell className="text-center">{formatCurrencyEGP(item.TotalValue)}</TableCell>
                    <TableCell className="text-center">{formatCurrencyEGP(item.TotalValueAfterOffer)}</TableCell>
                    <TableCell className="text-center">{item.ProductsCount}</TableCell>
                    <TableCell className="text-center">
