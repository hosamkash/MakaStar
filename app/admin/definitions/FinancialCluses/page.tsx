...
type FinancialClause = {
  id: string
  ID: string
  Code: string
  Name: string
  IsViewToRepresent: boolean
  IDFinancialType: number
  IsDefaultToTransferMony: boolean
  IsActive: boolean
}

type SortConfig = {
  key: keyof FinancialClause | null
...
export default function FinancialClausesPage() {
  ...
  const { settings, loading: loadingSettings, updateItemsPerPage, updateLastSelectedItem } = useScreenSettings("/admin/definitions/FinancialCluses")
  
  const [items, setItems] = useState<FinancialClause[]>([])
  ...
  const [selectedItem, setSelectedItem] = useState<FinancialClause | null>(null)
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setLoading(true)
        const itemsCollection = collection(db, "Def_FinancialCluses")
        ...
        const itemsData = itemsSnapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => {
          ...
          return {
            ...
            Name: data.Name || '',
            IsViewToRepresent: data.IsViewToRepresent || false,
            IDFinancialType: data.IDFinancialType || 0,
            IsDefaultToTransferMony: data.IsDefaultToTransferMony || false,
        ...
        console.error("Error fetching financial clauses:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "Def_FinancialCluses", id))
      setItems((prevItems) => prevItems.filter((item) => item.id !== id))
      setSelectedItem(null)
      notify.success("تم حذف البند المالي بنجاح")
    } catch (error) {
      console.error("Error deleting financial clause:", error)
      notify.error("حدث خطأ أثناء حذف البند المالي")
    }
  }
  ...
  const handleEdit = async (id: string) => {
    ...
    router.push(`/admin/definitions/FinancialCluses/form?id=${id}`)
  }

  const requestSort = (key: keyof FinancialClause) => {
  ...
  const SortableHeader = ({ columnKey, children }: { columnKey: keyof FinancialClause; children: React.ReactNode }) => (
  ...
  const actionButtons = [
    { label: "جديد", icon: Plus, href: "/admin/definitions/FinancialCluses/form" },
    { 
      label: "تعديل", 
      icon: Edit, 
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader title="عرض بنود المالية" actionButtons={actionButtons} />
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <SortableHeader columnKey="Name">الاسم</SortableHeader>
