...
interface PlaceSearchDialogProps {
  ...
  onPlaceSelect: (place: PlaceWithHierarchy) => void
}

export default function PlaceSearchDialog({ open, onOpenChange, onPlaceSelect }: PlaceSearchDialogProps) {
  ...
  const handlePlaceSelect = (place: PlaceWithHierarchy) => {
    onPlaceSelect(place)
    onOpenChange(false)
    setSearchTerm("")
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col">
        ...
        {/* Results Container - This is the scrollable area */}
        {!loading && (
          <div className="flex-1 overflow-y-auto overflow-x-hidden max-h-[400px] pr-2">
            <div className="space-y-3">
              {filteredPlaces.length === 0 ? (
                ...
                filteredPlaces.map((place) => (
                  <Card key={place.id} className="hover:shadow-md transition-shadow cursor-pointer border border-gray-200">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {place.Address && (
                            <div className="text-sm text-gray-500 mb-2">
                              <span className="font-medium">التفاصيل:</span> {place.Address}
                            </div>
                          )}
                          ...
                          {place.FullAddressText && (
                            <div className="text-xs text-gray-400">
                              <span className="font-medium">العنوان الكامل:</span> {place.FullAddressText}
                            </div>
                          )}
                        </div>
        ...
        {!loading && filteredPlaces.length > 0 && (
          <div className="text-center text-sm text-gray-500 border-t pt-3 flex-shrink-0">
            {searchTerm ? (
              <span>تم العثور على {filteredPlaces.length} مصلحة مطابقة</span>
            ) : (
              <span>مجموع محاكم السمسلاوين: {filteredPlaces.length} مصلحة</span>
            )}
          </div>
        )}
      </DialogContent>
